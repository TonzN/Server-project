                ...
                if group == "global":
                    for user in online_users:
                        if user != profile["name"]:
                            ...
                            response = json.dumps({"data": [{"user": "[global]"+profile["name"], "message": msg, "signals": "chat"}, "chat", ],  "signals": "chat"}) + "\n"
...
async def message_user(loop, data, tag, token):
    try:
        ...
        if profile:
            ...
            response = json.dumps({"data": [{"user": profile["name"], "message": msg, "signals": "chat"}, "chat"]}) + "\n"
            await client_socket.send(response.encode()) #to send other users messages you need their socket
